...
module.exports = (function ($) {
  ...
  var JustifiedGallery = function ($gallery, settings) {
    ...
    this.spinner = {
      phase: 0,
      timeSlot: 150,
      $container: $('<ark-box class="ark-preloader"></ark-box>'),
      intervalId: null
    };
  ...
  JustifiedGallery.prototype.displayEntryCaption = function ($entry) {
    ...
    if ($image !== null && this.settings.captions) {
      ...
      if ($imgCaption !== null) {
        if (!this.settings.cssAnimation) $imgCaption.hide();
  ...
  JustifiedGallery.prototype.prepareBuildingRow = function (isLastRow) {
    ...
    if (isLastRow && this.settings.lastRow === 'hide') {
  ...
  JustifiedGallery.prototype.flushRow = function (isLastRow) {
    ...
    if (isLastRow && settings.lastRow === 'hide') {
    ...
    for (var i = 0; i < this.buildingRow.entriesBuff.length; i++) {
      $entry = this.buildingRow.entriesBuff[i];
      this.displayEntry($entry, offX, this.offY, $entry.data('jg.jwidth'), this.buildingRow.height, minHeight);
  ...
  JustifiedGallery.prototype.getSpinnerHeight = function () {
    return this.spinner.$container.innerHeight();
  };
  ...
  JustifiedGallery.prototype.startLoadingSpinnerAnimation = function () {
    ...
    var $spinnerPoints = spinnerContext.$container.find('span');
    clearInterval(spinnerContext.intervalId);
    this.$gallery.append(spinnerContext.$container);
    this.$gallery.height(this.offY + this.getSpinnerHeight());
    spinnerContext.intervalId = setInterval(function () {
      if (spinnerContext.phase < $spinnerPoints.length) {
        $spinnerPoints.eq(spinnerContext.phase).show();
      } else {
        $spinnerPoints.eq(spinnerContext.phase - $spinnerPoints.length).hide();
      }
  ...
  JustifiedGallery.prototype.hideBuildingRowImages = function () {
    for (var i = 0; i < this.buildingRow.entriesBuff.length; i++) {
      if (this.settings.cssAnimation) {
        console.log("hieeee1111");
        this.buildingRow.entriesBuff[i].removeClass('entry-visible');
      } else {
        this.buildingRow.entriesBuff[i].hide();
      }
    }
  };
  ...
  JustifiedGallery.prototype.analyzeImages = function (isForResize) {
    ...
    for (var i = this.lastAnalyzedIndex + 1; i < this.entries.length; i++) {
      ...
      if ($entry.data('jg.loaded') === true || $entry.data('jg.loaded') === 'skipped') {
        ...
        if ((availableWidth / (this.buildingRow.aspectRatio + imgAspectRatio) <= this.settings.rowHeight) && this.buildingRow.entriesBuff.length >= this.settings.minimumInElementsRow) {
          flushedOnce = true;
          if(builtRows >= this.settings.rowsAmount)
          {
            for (var j = 0; j < this.buildingRow.entriesBuff.length; j++) {
              $(this.buildingRow.entriesBuff[j][0]).hide();
              console.log(this.buildingRow.entriesBuff[j][0]);
            }
            this.clearBuildingRow();
          }
          else{
    ...
    if (this.buildingRow.entriesBuff.length > 0) {
      for (var i = 0; i < this.buildingRow.entriesBuff.length; i++) {
        $(this.buildingRow.entriesBuff[i][0]).hide();
        console.log(this.buildingRow.entriesBuff[i][0]);
      }
      this.flushRow(flushedOnce);
      this.clearBuildingRow();
    }
   ...
   *
   * @param $image the image src to load
   * @param onLoad callback that is called when the image has been loaded
   * @param onError callback that is called in case of an error
   */
  JustifiedGallery.prototype.onImageEvent = function ($image, onLoad, onError) {
    ...
    $.each(this.entries, function (index, entry) {
      ...
      if ($entry.data('jg.loaded') !== true && $entry.data('jg.loaded') !== 'skipped') {
        ...
        if ($image !== null) {
          ...
          that.onImageEvent($image, function ($loadImg) { // image loaded
            // var aspectRatio = $loadImg.width()/$loadImg.height();
            // $loadImg.width($loadImg.height()/aspectRatio);
            // $loadImg.height(100);
          ...
          // var aspectRatio = $entry.width()/$entry.height();
          // $entry.width($entry.height()/aspectRatio);
          // $entry.height(100);
          $entry.data('jg.loaded', true);
